...
def forEvery(func=lambda x:x, elem=None):
    if hasattr(elem, "__iter__") and not isinstance(elem,str):
        return [forEvery(func, e) for e in elem]
    return func(elem)
import numpy,inspect
def forEveryWithIndex(elem, func=lambda e,i,v:e, axis=-1,for_all=False,clear_null=False, variables=None, index=()):
    ...
    if hasattr(elem, "__iter__") and not isinstance(elem,str) and len(index) != axis:
        ...
        for e in range(len(elem)):
            res=forEveryWithIndex(elem[e], func, axis, for_all,clear_null,variables,tuple(list(index) + [e]))
...
class Random(object):
    ...
    def uuid(samples=nm,num=2,group="default",limit=3000):
        ...
        if len(Random.uuidList[group])>=len(samples)**num/2:
            print("总共重新获取次数:",Random.count)
            Random.result.append(Random.count)
            Random.count=0
            Random.clear("default")
            return False

            raise RuntimeError("Id group was fulled.")
        count=0
        while count<limit:
